...
const { MediaInfoCommand } = require('/opt/mediainfo/');
const executeQuery = require('./executeQuery');
// Set the region

exports.handler = async (event) => {
  AWS.config.update({ region: event.awsRegion });
  console.log('InputLambda', JSON.stringify(event));
  if (event.Records[0].eventName.includes('ObjectCreated')) {
    await createJob(event.Records[0].s3);

    const metadata = await mediainfo(event.Records[0].s3);
    const key = path.parse(event.Records[0].s3.object.key).name;
    console.log(
      'UpdateVideoOnDemand',
      await executeQuery('UpdateVideoOnDemand', {
          input: {
              id: key,
...
async function mediainfo(eventObject) {
  const key = eventObject.object.key;
  const bucket = eventObject.bucket.name;
  ...
  const tracks = fullData.mediaInfo.media.track.reduce((res, track) => {
    res[track.$.type] = track;
    return res;
  }, {});
