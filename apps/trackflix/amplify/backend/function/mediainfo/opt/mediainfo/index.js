...
class MediaInfoError extends Error {
  constructor(...args) {
    ...
    this.message = `${this.errorCode} - ${
      this.message || 'unknown mediainfo error'
    }`
...
class MediaInfoCommand {
  ...
  get miniData() {
    return !this.jsonData
      ? undefined
      : {
          container: this.container.map((x) =>
            MediaInfoCommand.minifyPayload(x)
          ),
  ...
  get video() {
    return !this.jsonData
      ? undefined
      : ((this.jsonData.mediaInfo.media || {}).track || []).filter(
          (x) => x.$.type.toLowerCase() === 'video'
        )
  }
  ...
  static escapeS3Character(path) {
    ...
    const { AWSAccessKeyId, Signature } = url.query || {}
  ...
  async presign(params) {
    return new Promise((resolve, reject) => {
      ...
      if (typeof params === 'object' && (!params.Bucket || !params.Key)) {
        return reject(
          new Error(`missing Bucket and Key, ${JSON.stringify(params)}`)
        )
      }
  ...
  async command(url) {
    ...
    const ldLibraryPath = [config.LD_LIBRARY_PATH, process.env.LD_LIBRARY_PATH]
      .filter((x) => x)
      .join(':')
    const defaults = {
      cwd: undefined,
    ...
    const params = [...MediaInfoCommand.Constants.Command.Options, url]
  ...
  static strip(params, data) {
    ...
    const container = modified.mediaInfo.media.track.find(
      (x) => x.$.type.toLowerCase() === 'general'
    )
  ...
  static dedup(data) {
    function camelCaseKey(k0, obj) {
      const k1 =
        k0 === '_' || k0 === '$'
          ? k0
          : k0
              .replace(/^([A-Za-z])|[\s-_.]{1,}(\w)/g, (ignored, p1, p2) =>
    ...
    function flatten(a0) {
      if (
        !Array.isArray(a0) ||
        a0.findIndex((x) => typeof x === 'object') >= 0
      ) {
        return a0
      }
